...
**Exercise:** Try to use `map()`, from the `purrr::` package, to avoid copy
pasting?
:::
...
apply the function to all combinations of outcomes and exposures. Here are some
useful pieces of code that you can use:
...
-   `.f = function(x, y) build_model(x, y, dataset = nh2007)` is an anonymous function.     It helps pass a common element to all the models.
     Here, it's the dataset that we
     want to use for the models. Of course, this dataset can also be
     passed directly by `pmap` to make the dataset vary, or written in the
...
means that you can easily apply the function to new model results and
...
This report is only based on the 2007 data. 
Since the 2009 data in the `nh2009` dataset is very similar, we can reuse
the functions with it. It's very easy to replicate the analysis with new data:
...
**Exercice:** Integrate the following lines in the report document, and render
