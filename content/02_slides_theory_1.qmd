...
In this workshop: a folder on your computer where we will analyse the
NHANES data.
...
-   `stage`: Files and modification here are tracked by Git and can be
    put into the history with a `commit`. They still can be lost
    (unsafe).
...
::: callout-tip
Type `sd` in R (without parenthesis) to see how `sd()` calculate
standard deviation
:::

## Functions

-   The output of the function depends only on the inputs. Identical
    inputs will give identical results.
