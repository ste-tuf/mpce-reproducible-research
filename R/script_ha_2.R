...
#'  Cross tabulation of a variable as a dataframe
#'
#' @param variable 
compute_table <- function(variable) {
  table(
    variable,
    useNA = "always",
    dnn = "level" # Name for the first column of the dataframe
  ) |>
    as.data.frame()
}

#' Continuous variable summary statistics as a dataframe
#'
#' @param variable 
...
# Numbers
nh2007 |>
  purrr::map(.f = compute_descriptive_stats)
